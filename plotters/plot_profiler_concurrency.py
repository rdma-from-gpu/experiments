...
# Just a single model at a time
for model, mtimes in times.groupby("MODEL", sort=True):

    times = times.convert_dtypes()
    mtimes["CONCURRENCY"] = pd.to_numeric(mtimes["CONCURRENCY"])

    fig, ax = plt.subplots()
    for concurrency, cdata in mtimes.groupby("CONCURRENCY", sort=True):
    ...
    fig.tight_layout()
    fig.savefig(f"{output_path}/profiler_concurrency_cdf_{model}.pdf")
    ...
    fig.tight_layout()
    fig.savefig(f"{output_path}/profiler_concurrency_fitting_{model}.pdf")
